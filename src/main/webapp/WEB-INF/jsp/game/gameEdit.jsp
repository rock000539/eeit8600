	...
	<div class="container-fluid text-center">
		<div class="row content">
			<div class="col-sm-8 text-left">
				...
				<form class="form-horizontal col-sm-8" action="/games/update"
					method="post">
					<div class="form-group has-success">
						<label class="label label-default col-sm-2">編號：</label>
						<div class="col-sm-10">
							<div class="input-group">
								<input class="form-control input-sm" type="text" id="id"
									name="id" value="${game.id}" readonly="readonly">
								<div class="input-group-addon">
									<span id="span_id" class="glyphicon glyphicon-ok"></span>
								</div>
							</div>
						</div>
					</div>

					<div id="fg_name" class="form-group has-success">
						<label class="label label-default col-sm-2">名稱：</label>
						<div class="col-sm-10">
							<div class="input-group">
								<input class="form-control input-sm" type="text" id="name"
									name="name" data-type="string" value="${game.name}"
									placeholder="請輸入名稱">
								<div class="input-group-addon">
									<span id="span_name" class="glyphicon glyphicon-ok"></span>
								</div>
							</div>
						</div>
					</div>

					<div id="fg_evalu" class="form-group has-success">
						<label class="label label-default col-sm-2">評價：</label>
						<div class="col-sm-10">
							<div class="input-group">
								<input class="form-control input-sm" type="text" id="evalu"
									name="evalu" data-type="integer" value="${game.evaluation}"
									placeholder="請輸入評價">
								<div class="input-group-addon">
									<span id="span_evalu" class="glyphicon glyphicon-ok"></span>
								</div>
							</div>
						</div>
					</div>

					<div id="fg_dl" class="form-group has-success">
						<label class="label label-default col-sm-2">下載量：</label>
						<div class="col-sm-10">
							<div class="input-group">
								<input class="form-control input-sm" type="text" id="dl"
									name="dl" data-type="integer" value="${game.download}"
									placeholder="請輸入下載量">
								<div class="input-group-addon">
									<span id="span_dl" class="glyphicon glyphicon-ok"></span>
								</div>
							</div>
						</div>
					</div>

					<div id="fg_url" class="form-group has-success">
						<label class="label label-default col-sm-2">連結：</label>
						<div class="col-sm-10">
							<div class="input-group">
								<input class="form-control input-sm" type="text" id="url"
									name="url" data-type="string" value="${game.url}"
									placeholder="請輸入連結">
								<div class="input-group-addon">
									<span id="span_url" class="glyphicon glyphicon-ok"></span>
								</div>
							</div>
						</div>
					</div>
		...
		window.onload = function() {
			...
			btn_save.addEventListener("click",
					function() {
						...
						if (sp_list[1].className == "glyphicon glyphicon-ok" && sp_list[2].className == "glyphicon glyphicon-ok"
							&& sp_list[3].className == "glyphicon glyphicon-ok" && sp_list[4].className == "glyphicon glyphicon-ok") 
						{
							var data = '{"id":"' + $('#id').val() + 
								'", "name":"' + $('#name').val() + 
								...
								success:function(response){
									...
									if (response.redirect) {
								    	...
								    	for(var i=0; i<txt_list.length; i++){
								    		sp_list[i].className = "glyphicon glyphicon-question-ok";
								    		fg_list[i].className = "form-group has-success";
								    	}
								    }
								}, 
